...
description: |
  Master operational skill for engineering award-winning (Awwwards Site of the Year / FWA of the Day / CSSDA tier) creative web experiences.
  Synthesizes kinetic typography, synchronized GSAP 3 / ScrollTrigger / Lenis motion choreography, HTML5 Canvas 2D particle physics,
  Three.js / React Three Fiber (R3F) 3D scene graphs, custom GLSL visual computation shaders, and hybrid Blender pre-rendered frame sequence pipelines.
  Use when:
  - Designing and implementing award-winning digital experiences, brand showcases, and creative portfolios
  - Engineering interactive sand/dust text particle decomposition, cursor repulsion force fields, and elastic spring-back recovery
  - Choreographing pinned scrollytelling timelines with synchronized 3D model rotations, camera paths, or video scrubbers
  - Authoring GPU-accelerated vertex wave displacement, fragment liquid distortion, and mouse-velocity RGB chromatic aberration shaders
  - Unifying smooth scrolling (Lenis) with GSAP ScrollTrigger timelines and WebGL render loops under a singular master ticker
  - Implementing the ORYZO AI / Apple hybrid paradigm (offline Blender Cycles baking + 2D canvas frame scrubbing + real-time WebGL overlays)
  - Optimizing GPU/CPU performance, DPR clamping, InstancedMesh draw call budgets (<50 mobile, <100 desktop), and zero-allocation render loops
---
